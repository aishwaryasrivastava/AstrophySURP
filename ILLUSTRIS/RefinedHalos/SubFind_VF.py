...
for j in range(0,host_count):
	...
	for SatID in Satellites:
		...
		c.execute("SELECT SubhaloVel.X, Subhalos.StellarMass FROM Subhalos INNER JOIN SubhaloVel ON Subhalos.SubhaloID = SubhaloVel.SubhaloID WHERE Subhalos.SubhaloID = ?",(SatID,))
		result = c.fetchall()		
		SatVelX = result[0][0]
...
plt.savefig("plots/SubFind_SatVF.png")
plt.close()
