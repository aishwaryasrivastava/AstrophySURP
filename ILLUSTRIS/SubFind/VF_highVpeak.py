...
n_bins = int(sys.argv[1]) # 5 is a good choice
Vpeak = 12
...
for i in range(1,4):
	...
	for j in range(0,5):
		...
		for SatID in Satellites:
			c.execute("SELECT SubhaloVel.X FROM (SubhaloVel INNER JOIN Subhalos ON SubhaloVel.SubhaloID = Subhalos.SubhaloID) WHERE Subhalos.SubhaloID = ? AND Subhalos.SubhaloVmax > ?",(SatID,Vpeak))
			Result = c.fetchall()
			if len(Result) > 0:
				VelX = Result[0][0]
			else:
	...
	ax.set_title("Velocity function of hosts in range {index} with Vmax > {Vp}".format(index = i, Vp = Vpeak))
	plt.savefig("Range{index}/Range{index}_VF_highVpeak.png".format(index = i))
	plt.close()
conn.commit()
conn.close()
